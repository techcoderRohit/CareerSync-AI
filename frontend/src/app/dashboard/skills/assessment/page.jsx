...
export default function AssessmentPage() {
  ...
  const fetchQuizzes = async () => {
    try {
      ...
      const res = await axios.get("http://localhost:5000/api/skills/quizzes", {
  ...
  const startQuiz = async (skill) => {
    ...
    try {
      const token = localStorage.getItem("token");
      const res = await axios.get(`http://localhost:5000/api/skills/quiz/${skill}`, {
        headers: { Authorization: `Bearer ${token}` }
      });
  ...
  const submitQuiz = async () => {
    ...
    try {
      const token = localStorage.getItem("token");
      const res = await axios.post(`http://localhost:5000/api/skills/quiz/${selectedQuiz}/submit`, { answers }, {
        headers: { Authorization: `Bearer ${token}` }
      });
      setResult(res.data);
    } catch (err) {
